...
- top_caption:
  ...
  drawn under it. Accepts markdown. Defaults to `NULL`, which leaves out
  the footnote and, with it, the rule.
...
text color found in the rendered table (via
[`htmltools::as.tags()`](https://rstudio.github.io/htmltools/reference/as.tags.html)),
so it tracks the theme across light and dark color modes, and falls back
to a neutral gray when the render carries no color. The rules are keyed
on the table id. If the table has none, one is generated and assigned.
