...
Roboto Condensed throughout, with bold column labels and zebra-striped
rows under a centered heading, after Baseball Savant. Row groups render
as off-white labels on a black fill, and column spanners are bold and
underlined.
...
The heading is centered rather than left aligned. Rows are zebra-striped
through
[`gt::opt_row_striping()`](https://gt.rstudio.com/reference/opt_row_striping.html)
and horizontal rules are hidden. The last body row's bottom border is
painted white so it does not double the rule that closes the table.
