...
# The top 100 skaters by points, keyed by NHL API player ID
player_stats <- fastRhockey::nhl_stats_skaters(season = season_id, limit = 100)
```
...
The NHL's own APIs, which fastRhockey's `nhl_*()` functions read, identify
players by their NHL API ID (McDavid is 8478402), not their ESPN athlete ID,
which is why these headshots pass `id_type = "league"`: it draws NHL API IDs
from the NHL's image CDN. Leave `id_type` unset for ESPN athlete IDs, such as
those in fastRhockey's `espn_nhl_*()` and `load_nhl_*()` data. Both are plain
