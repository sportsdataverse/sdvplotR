...
`sdvplotR` ships with a function that makes creating tier lists convenient and quick. However, you need to pass your data to `gt` in a specific format. This vignette walks through how to create a simple tier list.
...
All done! You've created a tier list in `gt` using `sdvplotR`. The tier list function is somewhat limited: a) it only supports image cells and b) it will not "wrap" your logos to condense the width (you can play around with image_height to do this).
