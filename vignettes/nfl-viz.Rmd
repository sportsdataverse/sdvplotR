---
title: "NFL Visualizations with nflfastR and sdvplotR"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{NFL Visualizations with nflfastR and sdvplotR}
...
This vignette demonstrates how to create rich NFL visualizations by combining
[nflfastR](https://www.nflfastr.com/) for data ingestion with
...
```{r load-pbp}
# The last completed season: from September on, the season under way is too
# young for the per-play and per-passer cutoffs below
season <- nflreadr::most_recent_season() -
  (as.integer(format(Sys.Date(), "%m")) >= 9)
...
Create a scatter plot showing team EPA (Expected Points Added) per play, with
team logos replacing the data points:
...
Replace axis labels with team logos using `scale_x_sdv()`:
