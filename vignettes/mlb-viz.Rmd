---
title: "MLB Visualizations with baseballr and sdvplotR"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{MLB Visualizations with baseballr and sdvplotR}
...
This vignette demonstrates how to create rich MLB visualizations by combining
[baseballr](https://billpetti.github.io/baseballr/) for player and team data with
...
Use `baseballr` to load team and player statistics from the MLB Stats API:
...
The MLB Stats API and Baseball Savant identify players by their MLBAM ID, not
their ESPN athlete ID, which is why these headshots pass `id_type = "league"`:
it draws MLBAM IDs from MLB's own image CDN. Leave `id_type` unset for ESPN
athlete IDs, such as those in baseballr's `espn_mlb_*()` data. Both are plain
...
division_map <- teams |>
  ...
  mutate(team_rank = row_number(), .by = division_num)
...
comparison <- bind_rows(
  ...
  mutate(rank = row_number(), .by = category)
...
- Explore [baseballr documentation](https://billpetti.github.io/baseballr/)
