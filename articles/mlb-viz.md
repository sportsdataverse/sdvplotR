...
baseballr’s MLB Stats API and Baseball Savant functions identify players
by their MLBAM ID, not their ESPN athlete ID, which is why the headshots
above pass `id_type = "league"`: it draws MLBAM IDs from MLB’s own image
CDN. Leave `id_type` unset for ESPN athlete IDs, such as those in
baseballr’s `espn_mlb_*()` data. Both are plain numbers, so the wrong
...
ggplot(hitters, aes(x = games_played, y = home_runs)) +
  geom_sdv_headshots(
    aes(player_id = player_id),
    sport = "mlb",
    id_type = "league",
  ...
  labs(title = "MLB home run leaders", x = "Games Played", y = "Home Runs")
```
