...
double it into one row per team with
[`nflreadr::clean_homeaway()`](https://nflreadr.nflverse.com/reference/clean_homeaway.html),
and keep the SEC. `clean_homeaway()` renames the `home_*` / `away_*`
pairs to `team_*` / `opponent_*` based on where each team played, so the
same `team_id` we use for the opponent’s logo also gives us each team’s
own logo.
...
data <- read_csv("https://github.com/sportsdataverse/cfbfastR-data/raw/refs/heads/main/schedules/csv/cfb_schedules_2025.csv") %>%
  ...
  select(
    home_team, away_team, week, home_conference, away_conference,
    home_id, away_id, neutral_site
  ) %>%
  nflreadr::clean_homeaway() %>%
  filter(team_conference == "SEC") %>%
  mutate(
    opponent_logo = paste0(base, opponent_id, ".png"),
...
logos <- data %>%
  arrange(team) %>%
  pivot_wider(
    id_cols = c(team, team_logo), names_from = week,
...
[`arrange()`](https://dplyr.tidyverse.org/reference/arrange.html). Each
mask is then a `TRUE`/`FALSE` grid the same shape as the week columns,
which is what
[`gt_highlight_cells()`](https://sdvplotR.sportsdataverse.org/reference/gt_highlight_cells.md)
takes.
...
gt_legend_discrete(
  c(
    "Home Game" = "#FBE7C6", "Away Game" = "#FFFFFF",
  ...
  heading_style = list(
    font = "Oswald", size = 36, weight = 600,
