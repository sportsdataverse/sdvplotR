...
draws each value against the field it belongs to, and
[`gt_cutline()`](https://sdvplotR.sportsdataverse.org/reference/gt_cutline.md)
marks a break between rows. This vignette builds one table with each,
using flight data from
[`nycflights13`](https://github.com/tidyverse/nycflights13).

## Percentile bars

We start with the 15 busiest destinations out of NYC and their average
arrival delay. Alongside the raw delay we compute a percentile, so each
destination can be read against every other qualifying airport, not just
the 15 shown.
...
`cume_dist(-avg_delay)` gives the percentile of each destination’s
delay, and the minus sign flips it so a low delay is a high percentile
(a good result). The value comes out on a 0-1 scale, which matters in a
moment.

Now the table.
[`gt_percentile_bar()`](https://sdvplotR.sportsdataverse.org/reference/gt_percentile_bar.md)
turns the percentile column into a filled track with a numbered marker
at the tip.
...
percentile axis, so a raw `0.71` prints as `71`. A column already on a
0-100 scale is left alone, so the same code works either way. Set
`scale = "none"` to turn that off.
...
- `full_track`: whether the unfilled track runs the full width or stops
  at the marker. We set it to `FALSE` here so each bar ends at its own
  value.
- `marker_size`, `track_height`, `text_color`, `decimals`: the size of
  the marker, the thickness of the track, the color of the number inside
  the marker, and its decimal places.
...
The second table asks a different question: of the destinations with
real volume, which had the worst arrival delays and which had the best?
We take the 8 highest and 8 lowest average delays and stack them in one
sorted table.
...
`slice(c(1:8, (n() - 7):n()))` keeps the first 8 and last 8, already
...
d2 %>%
  ...
  gt_538_caption(
    md('Install using ... pak::pak("sportsdataverse/sdvplotR")'),
...
the highest delays give way to the lowest. The label sits on the line,
rendered in uppercase.

The first call, `gt_cutline(after = 0, ...)`, is the useful trick.
`after = 0` draws a line above the very first row, which lets you label
the top section the same way you label the break in the middle.
...
- `gap`: extra space around the line, in pixels. A single number pads
  ...
  Here `gap = c(12, 0)` opens room above the “Lowest” line without
  pushing the row below it down.

The two [`tab_style()`](https://gt.rstudio.com/reference/tab_style.html)
calls color the delay values to match their group, red for the high
block and green for the low, so the split reads even without the labels.
Note `cell_text(size = px(14))` on the body: the `gt_theme_*` functions
set their font sizes per cell, so a `tab_options(table.font.size = ...)`
will not move them. Restyle the body cells directly instead, which is
what that line does.
...
leaves the table flowing and marks a fixed row number.
