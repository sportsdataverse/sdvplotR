...
[`html_table()`](https://rvest.tidyverse.org/reference/html_table.html)
reads the spanner as the column names and leaves the true header sitting
in row one (`row_to_names(1)`). Promoting row one fixes it, and
[`clean_names()`](https://sfirke.github.io/janitor/reference/clean_names.html)
then turns `W` and `L` into names you can use without backticks.
...
happens inside each window with
...
win percentage breaks ties on total wins.
...
gt_legend_discrete(
  c(
    "No Titles In Window" = "#ffffff", "1 Title In Window" = "#F4D7A4",
  ...
  heading_style = list(
    font = "Spline Sans Mono", size = 24, weight = 600,
    transform = "uppercase", margin_bottom = 0, align = "center"
  ),
  subtitle_style = list(
    font = "Spline Sans Mono", size = 12, margin_bottom = 5,
